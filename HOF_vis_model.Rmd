...
n <- dim(data)[1]
sel <- data[, "Votes"] == 1 & data[, "WAR"] < 15 & !is.na(data[, "Votes"])
out <- data[sel, c("Name", "Year", "WAR", "Votes", "NumBallots")][1:10, ]
rownames(out) <- NULL
out
```
</li>
...
```{r edaplots2, eval=TRUE, echo=FALSE, fig.width=10, fig.height=5, out.width=900}
data <- read.csv(file="HOFregression_updated.csv", as.is=TRUE)
par(mfrow=c(1, 2))
sel <- data[, "Name"] == "Alan Trammell"
